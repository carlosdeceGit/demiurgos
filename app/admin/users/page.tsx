...
export default async function AdminUsersPage() {
  ...
  const [authResult, profilesResult, runsResult] = await Promise.all([
    ...
    admin
      .from("profiles")
      .select(
        "user_id, display_name, status, usage_limit, spend_limit, blocked_reason, blocked_by, blocked_at, onboarding_completed"
      ),
    admin.from("ai_runs").select("user_id, model, tokens, cost, created_at"),
  ]);
